...
Nesta solução, inicialmente importamos as classes `datetime` e `timedelta` do módulo `datetime`. A função `datetime.now()` é utilizada para obter a data e a hora atuais que são atribuídas à variável `data\_atual`. A classe `timedelta` é usada para representar uma duração, a diferença entre duas datas ou tempos. Adicionamos 30 dias à data atual utilizando a operação de soma com o objeto `timedelta` que é inicializado com o argumento `days=30`. A data resultante é atribuída à variável `data\_futura`. Finalmente, as duas datas são impressas.
