...
Crie uma classe chamada `Fracao` com um método `\_\_init\_\_` que inicialize o numerador e o denominador da fração. Crie um método chamado `mostrar\_dados` que exiba a fração no formato "numerador/denominador". Crie um método chamado `multiplicar` que receba outra instância da classe `Fracao` como parâmetro e retorne uma nova instância da classe `Fracao` que represente a multiplicação das duas frações. Crie uma instância da classe `Fracao` e chame o método `mostrar\_dados` e o método `multiplicar` com outra instância da classe `Fracao` como parâmetro.
