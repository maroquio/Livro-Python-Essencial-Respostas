...
Neste código, as linhas do arquivo são invertidas usando a notação de slice [::-1] e armazenadas na lista `linhas\_invertidas`. Em seguida, um novo arquivo chamado "arquivo\_invertido.txt"\ é criado em modo de escrita ("w") e as linhas invertidas são escritas nele usando o método `writelines`.
